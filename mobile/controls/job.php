...
if($_REQUEST['a']=='index'){
	...
	$where='';
	if($_GET['user']==1){//我的任务
		$signArr=$jobsignModel->field("id,jid")->where(" uid='".$_SESSION['tyuser']['id']."' ")->dataArr();
		if($signArr){
			$signRow=array();
	...
	if($where){
		$listArr = $model->where($where)->order('ordernum desc,id desc')->limit($limitStr)->dataArr();
	}else{
		$listArr = $model->order('ordernum desc,id desc')->limit($limitStr)->dataArr();
	}
