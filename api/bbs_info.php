...
class bbs_info{
	...
	public function getBbs(){
		$applistbbsModel = new Model_ApplistBbs();
		$filter['where'] = " customer_id='$this->customer_id' and state='1' ";
		$sql=$applistbbsModel->select($filter);
		$inforesult = $applistbbsModel->fetchRow($sql);
		$data['msgtype']	= 'news';
		...
		$data['url'] = 'http://'.$_SERVER['HTTP_HOST'] . "/mobile/bbs/index.php?fromuser=".$this->fromuser."&sign=".$this->timesign;
