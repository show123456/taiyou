...
class questionnaire_info{
	...
	public function getData($result){
		...
		if($this->info_type=='questionnaire'){
	...
	public function getquestionnaire(){
		$applistquestionnaireModel = new Model_Applistquestionnaire();
		$filter['where'] = " customer_id='$this->customer_id' and state='1' ";
		$sql=$applistquestionnaireModel->select($filter);
		$inforesult = $applistquestionnaireModel->fetchRow($sql);
		$data['msgtype']	= 'news';
		...
		$data['url'] = 'http://'.$_SERVER['HTTP_HOST'] . "/mobile/questionnaire/index.php?fromuser=".$this->fromuser."&sign=".$this->timesign;
