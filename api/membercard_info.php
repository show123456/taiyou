...
class membercard_info{
	...
	public function getData($result){
		...
		if($this->info_type=='membercard'){
			return $this->getMembercard();
		}
	}

	//预约单图文回复
	public function getMembercard(){
		$applistmembercardModel = new Model_ApplistMembercard();
		$filter['where'] = " customer_id='$this->customer_id' and state='1' ";
		$sql=$applistmembercardModel->select($filter);
		$inforesult = $applistmembercardModel->fetchRow($sql);
		$data['msgtype']	= 'news';
		...
		$data['url'] = 'http://'.$_SERVER['HTTP_HOST'] . "/mobile/membercard/index.php?fromuser=".$this->fromuser."&sign=".$this->timesign;
