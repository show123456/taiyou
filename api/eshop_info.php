...
class eshop_info{
	...
	public function getEshop(){
		$applisteshopModel = new Model_ApplistEshop();
		$filter['where'] = " customer_id='$this->customer_id' and state='1' ";
		$sql=$applisteshopModel->select($filter);
		$inforesult = $applisteshopModel->fetchRow($sql);
		$data['msgtype']	= 'news';
		...
		$data['url'] = 'http://'.$_SERVER['HTTP_HOST'] . "/mobile/eshop/".$inforesult['template']."/index.php?fromuser=".$this->fromuser."&sign=".$this->timesign;
		return $data;
	}
