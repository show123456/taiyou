...
if($_REQUEST['a']=='index'){
	...
	if($_GET['p']){
		...
		$smarty->setLayout('')->setTpl('home/task/templates/task_company_index.html')->display();die;
	}
}
//企业的报名管理（未结算的职位）
if($_REQUEST['a']=='subindex'){
	...
	if($_GET['p']){
		...
		$smarty->assign('list',$listArr);
		$smarty->setLayout('')->setTpl('home/task/templates/task_company_subindex.html')->display();die;
	}
}
//报名列表
...
if($_REQUEST['a']=='sign_js'){
	...
	if(method_is('post')){
		...
		$taskRow=$model->field('id,pay_type')->where("id=".$data['tid'])->dataRow();
		//1现金日结，用户金额不增加，2转账日结，用户金额增加
		if($taskRow['pay_type']==2){
			$listArr=$signModel->where("is_js!=2 and tid=".$data['tid'])->dataArr();
			foreach($listArr as $k=>$v){
				//用户金额增加
				$uRow_m=$userModel->find($v['uid']);//用户行
				$userData['info']['id']=$uRow_m['id'];
				$userData['info']['money']=$uRow_m['money']+$v['fact_money'];
				$userModel->add($userData);
				//写金额日志
				$logData['info']['uid']=$uRow_m['id'];
		...
		$smarty->assign('list',$listArr);
		$smarty->setLayout('')->setTpl('home/task/templates/sign_js.html')->display();die;
	}
}
if($_REQUEST['a']=='sign_js_ajax'){
	$signModel=new Model_Subtable('sub_sign');
	$pageSize=30;$p=$_GET['p'];$limitStr = ($p-1)*$pageSize.','.$pageSize;
	$listArr=$signModel->where("is_qd=1 and tid=".$_GET['tid'])->limit($limitStr)->dataArr();
	if($listArr){
		foreach($listArr as $key=>$value){
			//从快照中获取用户信息
			$value['user_json'] ? $uRow=unserialize($value['user_json']) : $uRow=$userModel->find($value['uid']);
...
if($_REQUEST['a']=='sign_mid'){
	...
	if($taskRow['is_js']==1){
		...
		$smarty->setLayout('')->setTpl('home/task/templates/sign_index_js.html')->display();die;
	}else{
		$smarty->assign('tid',$_GET['tid']);
		$smarty->setLayout('')->setTpl('home/task/templates/sign_mid.html')->display();die;
	}
}
//签到页
if($_REQUEST['a']=='sign_qd'){
	...
	if(method_is('post')){
		...
		$smarty->assign('list',$listArr);
		$smarty->setLayout('')->setTpl('home/task/templates/sign_qd.html')->display();die;
	}
}
if($_REQUEST['a']=='sign_qd_ajax'){
	$signModel=new Model_Subtable('sub_sign');
	$pageSize=30;$p=$_GET['p'];$limitStr = ($p-1)*$pageSize.','.$pageSize;
	$listArr=$signModel->where("is_valid=1 and tid=".$_GET['tid'])->limit($limitStr)->dataArr();
	if($listArr){
		foreach($listArr as $key=>$value){
			//从快照中获取用户信息
			$value['user_json'] ? $uRow=unserialize($value['user_json']) : $uRow=$userModel->find($value['uid']);
